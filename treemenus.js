...
ddtreemenu.createTree = function(treeid, enablepersist, persistdays) {
    var ultags =
	document.getElementById(treeid).getElementsByTagName("ul")
    
    if (typeof persisteduls[treeid] == "undefined")
	persisteduls[treeid]
	= (enablepersist == true && ddtreemenu.getCookie(treeid) != "")
	? ddtreemenu.getCookie(treeid).split(",")
	: ""
	...
	var durationdays
	    = (typeof persistdays == "undefined")
	    ? 1
	    : parseInt(persistdays)
	
	ddtreemenu.dotask(window,
			  function() {
			      ddtreemenu.rememberstate(treeid,
						       durationdays)
			  },
...
ddtreemenu.buildSubTree = function(treeid, ulelement, index) {
	...
	if (ddtreemenu.searcharray(persisteduls[treeid], index)) {
	    ...
	    ulelement.setAttribute("rel", "closed")
	     ...
	     || ulelement.getAttribute("rel") == false) // if no cookie and UL has NO rel attribute explicted added by user
	ulelement.setAttribute("rel", "closed")
...
ddtreemenu.flatten = function(treeid, action) { // expand or contract all UL elements
	...
	ultags[i].parentNode.style.backgroundImage
	    = (action == "expand")
	    ? "url(" + ddtreemenu.openfolder + ")"
...
ddtreemenu.rememberstate = function(treeid, durationdays) {
    // Store index of opened ULs relative to other ULs in Tree into
    // cookie.
	...
	if (ultags[i].getAttribute("rel") == "open") {
	    ...
	    // opened UL (relative to the
	    // entire list of ULs) as an
	    // array element.
	}
    }
    if (openuls.length == 0) {
	// if there are no opened ULs to save/persist.
	openuls[0] = "none open" // set array value to string to
	// simply indicate all ULs should
	// persist with state being closed.
    }
    ddtreemenu.setCookie(treeid, openuls.join(","), durationdays)
    // Populate cookie with value treeid=1,2,3 etc (where 1,2... are
    // the indexes of the opened ULs).
}
...
ddtreemenu.getCookie = function(Name) {
    ...
    // Construct RE to search
    // for target name/value pair 
